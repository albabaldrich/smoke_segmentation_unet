...
def DataAugmentation(Dataset, cfg):
    ...
    train_augment = A.Compose([#A.PadIfNeeded(min_height=256, min_width=256, p=1),
                                A.RandomResizedCrop (width=cfg['img_width'], height=cfg['img_height'], scale=[0.07, 1.0], ratio=(1, 1), p=0.8),
                                ##A.RandomCrop(height=cfg['img_height']/4, width=cfg['img_width']/2, p=0.1),
                                # A.ElasticTransform(alpha=10, sigma=150, alpha_affine=50, interpolation=1, border_mode=4, value=17, mask_value=105, p=1),
                                A.GaussNoise(p=0.2),
                                A.Blur(p=0.1),
                                ##A.VerticalFlip(p=1),
    ...
    #image1, mask1 = Dataset['images'][0], Dataset['masks'][0]
    for image,mask in zip(Dataset['images'], Dataset['masks']):
        done = False
        while (not done):
            try: 
...
#    TrainDataset = {'images':train_images[0:1], 'masks':train_masks[0:1]}
...
def ShowDatasetExample(Augmented, Dataset=None, i=0):
